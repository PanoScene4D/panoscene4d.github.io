...
class StatusBar extends Container {
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const frameValue = createStat('frame');
        ...
        let splat: Splat;
        let totalFrames = events.invoke('timeline.frames') as number ?? 0;
        let currentFrame = events.invoke('timeline.frame') as number ?? 0;
        const formatFrame = () => totalFrames > 0 ? `${currentFrame + 1}/${totalFrames}` : '0/0';
        ...
        const updateTimelineStats = () => {
            frameValue.text = formatFrame();
        };
        ...
        events.on('timeline.frames', (frames: number) => {
            totalFrames = frames;

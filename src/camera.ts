...
class Camera extends Element {
    ...
    renderOverlays = true;
    xrDirectRender = false;
    private xrSavedRenderOverlays = true;
    private xrSavedGizmoLayerEnabled = true;
    private xrSavedWorldTransparentSortMode: number | null = null;
    ...
    onUpdate(deltaTime: number) {
        if (this.xrDirectRender || this.scene.app.xr?.active) {
            // Keep desktop debug locomotion active in XR. The controller moves the
            // camera root rig rather than the headset-owned camera transform.
            this.controller.update(deltaTime);
            this.fitClippingPlanes(this.mainCamera.getPosition(), this.mainCamera.forward);
            this.mainCamera.camera.camera._updateViewProjMat();
            return;
        }
    ...
    startXrDirectRender() {
        ...
        this.xrDirectRender = true;
        this.xrSavedRenderOverlays = this.renderOverlays;
        this.xrSavedGizmoLayerEnabled = this.scene.gizmoLayer.enabled;
        this.xrSavedWorldTransparentSortMode = this.scene.worldLayer.transparentSortMode;

        this.renderOverlays = false;
        this.scene.gizmoLayer.enabled = false;
        this.scene.worldLayer.transparentSortMode = SORTMODE_MANUAL;
        this.camera.renderPasses = null;
        this.camera.clearColor.copy(this.scene.events.invoke('bgClr'));
        const splats = this.scene.getElementsByType(ElementType.splat) as Splat[];
        splats.forEach((splat, index) => {
            splat.setXrDirectRender(true, splat.getXrDrawOrder(index));
            splat.entity.gsplat.instance.sort(this.mainCamera);
        });
    ...
    endXrDirectRender() {
        ...
        this.xrDirectRender = false;
        this.renderOverlays = this.xrSavedRenderOverlays;
        this.scene.gizmoLayer.enabled = this.xrSavedGizmoLayerEnabled;
        if (this.xrSavedWorldTransparentSortMode !== null) {
            this.scene.worldLayer.transparentSortMode = this.xrSavedWorldTransparentSortMode;
            this.xrSavedWorldTransparentSortMode = null;
        }
        (this.scene.getElementsByType(ElementType.splat) as Splat[]).forEach(splat => splat.setXrDirectRender(false));
        this.rebuildRenderTargets();
        this.scene.forceRender = true;
    }

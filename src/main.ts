...
const loadSequenceManifest = async (events: Events, manifestUrl: string) => {
    ...
    const importFiles = frames.map((frame, frameIndex) => {
        const resolvedUrl = new URL(frame.url, response.url).toString();
        const fallbackName = resolvedUrl.split('/').pop() ?? `frame_${frameIndex}.ply`;
        return {
            filename: frame.filename ?? fallbackName,
            url: resolvedUrl
        };
    });
...
const main = async () => {
    ...
    const xrHud = document.createElement('div');
    xrHud.id = 'xr-hud';
    xrHud.className = 'xr-hud';

    const renderFpsMeter = document.createElement('div');
    renderFpsMeter.id = 'render-fps-meter';
    renderFpsMeter.className = 'xr-hud-control fps-meter';
    renderFpsMeter.textContent = '0.0 FPS';
    xrHud.appendChild(renderFpsMeter);

    const vrButton = document.createElement('button');
    vrButton.type = 'button';
    vrButton.id = 'vr-button';
    vrButton.className = 'xr-hud-control vr-button';
    vrButton.textContent = 'Checking VR…';
    vrButton.disabled = true;
    xrHud.appendChild(vrButton);
    document.body.appendChild(xrHud);

    let renderFpsSamples = 0;
    let renderFpsElapsed = 0;
    const formatRenderFps = (value: number) => `${(value > 0 ? value : 0).toFixed(1)} FPS`;
    events.on('update', (dt: number) => {
        renderFpsSamples += 1;
        renderFpsElapsed += dt;

        if (renderFpsElapsed >= 0.5) {
            renderFpsMeter.textContent = formatRenderFps(renderFpsSamples / renderFpsElapsed);
            renderFpsSamples = 0;
            renderFpsElapsed = 0;
        }
    });

    const xr = scene.app.xr;
    const onVrAvailabilityChange = (available: boolean) => {
        vrButton.disabled = !available;
        vrButton.textContent = available ? (xr?.active ? 'Exit VR' : 'Enter VR') : 'VR unavailable';
    };

    if (xr) {
        xr.on('available:immersive-vr', onVrAvailabilityChange);
        xr.on('start', () => {
            scene.camera.startXrDirectRender();
            vrButton.textContent = 'Exit VR';
        });
        xr.on('end', () => {
            scene.camera.endXrDirectRender();
        ...
        vrButton.addEventListener('click', () => {
            try {
                ...
                const xrScaleParam = Number(new URLSearchParams(window.location.search).get('xrScale'));
                const framebufferScaleFactor = Number.isFinite(xrScaleParam) && xrScaleParam > 0 ?
                    Math.max(0.4, Math.min(1, xrScaleParam)) :
                    0.7;
                scene.camera.mainCamera.camera.startXr('immersive-vr', 'local-floor', {
                    framebufferScaleFactor
                } as any);
            } catch (error) {
                console.error('Failed to start immersive-vr session', error);
                vrButton.textContent = 'VR start failed';
            }
        });

        onVrAvailabilityChange(Boolean((xr as any).isAvailable?.('immersive-vr')));
    } else {

...
const ensureFileContents = async (file: ImportFile): Promise<File> => {
    if (file.contents) {
    ...
    if (file.url) {
        ...
        return new File([blob], file.filename, { type: blob.type || 'application/octet-stream' });
    }

    throw new Error(`No file contents for '${file.filename}'`);
};

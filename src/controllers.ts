...
class PointerController {
    ...
    constructor(camera: Camera, target: HTMLElement) {
        ...
        this.update = (deltaTime: number) => {
            ...
            if (camera.scene.app.xr?.active) {
                // During XR, the headset pose owns the camera transform. Move the shared
                // parent rig instead so mouse/head tracking remains intact while WASD
                // translates the viewer through the scene.
                const rigPosition = camera.scene.cameraRoot.getPosition();
                camera.scene.cameraRoot.setPosition(rigPosition.add(moveVec));
                return;
            }

            if (camera.controlMode === 'fly') {
                // Desktop fly mode: move the focal point (camera follows due to orbit calculation).
                const p = camera.focalPoint.add(moveVec);
                camera.setFocalPoint(p);
            }
        };

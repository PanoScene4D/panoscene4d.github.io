...
class Splat extends Element {
    ...
    xrDirectMaterial = false;
    xrSavedLayers: number[] | null = null;
    xrSavedCull = true;
    xrSavedDrawOrder = 0;
    ...
    async add() {
        ...
        if (this.scene.camera?.xrDirectRender) {
            this.setXrDirectRender(true);
        }
    ...
    getXrDrawOrder(index = 0) {
        const filename = this.filename.toLowerCase();
        if (/(^|[/_-])frame[_-]?\d+/.test(filename) || filename.includes('dynamic')) {
            return 1000 + index;
        }
        if (filename.includes('background') || filename.includes('static') || filename.includes('_bg') || filename.startsWith('bg')) {
            return index;
        }
        return 100 + index;
    }

    setXrDirectRender(enabled: boolean, drawOrder = this.getXrDrawOrder()) {
        const { instance } = this.entity.gsplat;
        const { material } = instance;
        const meshInstance = instance.meshInstance;

        if (this.xrDirectMaterial === enabled) {
            if (enabled) {
        ...
        if (enabled) {
            this.xrSavedLayers = this.entity.gsplat.layers.slice();
            this.xrSavedCull = meshInstance.cull;
            this.xrSavedDrawOrder = meshInstance.drawOrder;

            meshInstance.cull = false;
            meshInstance.drawOrder = drawOrder;
            this.entity.gsplat.layers = [this.scene.worldLayer.id];

            const { glsl } = material.shaderChunks;
            glsl.delete('gsplatVS');
            glsl.delete('gsplatPS');
            glsl.delete('gsplatCenterVS');
            instance.configureMaterial(material);
            material.setDefine('SH_BANDS', `${(instance.resource as any).shBands ?? 0}`);
            material.setParameter('alphaClip', 1 / 255);
            material.update();
            this.xrDirectMaterial = true;
        } else {
            meshInstance.cull = this.xrSavedCull;
            meshInstance.drawOrder = this.xrSavedDrawOrder;
            this.entity.gsplat.layers = this.xrSavedLayers ?? [this.scene.splatLayer.id];
            this.xrSavedLayers = null;
            this.rebuildMaterial(this.scene.events.invoke('view.bands'));
        }
